...
> **版本**: v3.13 — 2026-09-26（新增 #252-#275：加载器把失败伪装成成功的四连击（PI0 strict=True 仍返回未训练模型 / pretrained_path 不回填导致归一化统计量为空 / groot 裸 except / wall_x 不报 key）、sync 与 RTC 关节顺序置换在双臂上复合成 runaway、官方 smolvla_libero 自带 n_action_steps=50 吃掉 20-23pp、--env.fps 静默改仿真控制率、lerobot-record 存请求动作而非实际动作、StreamingLeRobotDataset 跨文件时间戳静默配错帧、Figure Helix 2.5 的 9%→56% 预训练消融 + 30 户未见住宅零样本、Skild「慢 10 倍就是不可部署」+ $100M ARR、FLUX 3 Action 7B 开源 WAM + RoboLab-120 排行 + 用游戏当评测台、Isaac ROS CUDA 零拷贝的四个静默回退条件、Isaac-GR00T 部署六条硬约束 + 全平台 TRT 实测（代价：闭环 100%→95%）、GR00T N2 未发布的纠偏、SmolVLA ONNX 掉 29pp 的真凶是 16-token 语言宽度截断（且那个"INT8"是 FP32）、VLAQuantBench 多量化 41 层反涨 63.5pp、EffVLA「action head 由初始化决定」、FailBench 接触任务不超过 0.60、GE-Act 2.0 的 100× 数据只买 2.6× 成功率、vla.simd 的 90 秒热浸泡、RoboDrop / TraceFlow 数据侧两把工具，以及 17 条摘要级 arXiv 速查）
> **上版**: v3.12 — 2026-08-28（新增 #230-#251：LeRobot 相机漂移实测 37px/两分钟且事后不可恢复、微调 `input_features` 不刷新导致 24/251 公开 checkpoint 配置自相矛盾（7 个官方）、Python 3.14 全面炸掉 `from_pretrained`、同数据 SmolVLA 打赢 Pi0.5 的真机对照、RDA 数据集审计 CLI、TOWN-VLA prompt-form collapse 92.47%→3.00%、FlashVLA 30Hz 流式解码、UCAG-P 相机中心统一动作空间、LM-X 2 万小时含 1000 小时失败数据、TacForcing 执行期触觉、M3 模态遮蔽 +21.7%、ROS2SmolVLA 工业臂、PredVLA 0.68M 参数、FLARE 失败恢复、InstructMove 文本不可省 benchmark、StreamPI/UniMem/TemporalFlow 记忆三件套、GVLA 多夹爪 10.3 万演示、TrapVLA/CertVLA 安全线，以及 19 条摘要级 arXiv 速查）
...
> **采集 skill**: `.qoder/skills/en-vla-collector.md`（注：CLAUDE.md 与定时任务引用的 `scripts/en-vla-collector/english-vla-collector-SKILL.md` 不存在，v3.13 起以 .qoder 路径为准）
...
| 252 | LeRobot 本期**没有发布**，但 `pyproject.toml` 已是 0.6.2——版本字符串 ≠ 已发布 wheel | GitHub lerobot | 2026-09-25 | Debug | [链接](https://github.com/huggingface/lerobot/releases) | **读 bug 报告前先搞清对方跑的是什么**。v0.6.1（2026-08-03，commit `7e241bd`）到本期结束仍是 `Latest`，**2026-08-28 → 09-25 窗口内零发布**（延续 #211）。但 main 的 `pyproject.toml` 已声明 **0.6.2**，PyPI 最新仍是 v0.6.1——所以本期一批 issue 自报 "Version 0.6.2"（如 #4577）指的是 **main，不是任何发布版**，`pip show lerobot` / `lerobot.__version__` 会和最新 tag 不一致。**复现别人的 bug 时必须问清是 tag 还是 main**。另：v0.6.1 的共享 VLA 组件大重构（pi0 #4062 / pi05 #4063 / pi0_fast #4055 / eo1 #4061 / smolvla #4064）+ `refactor(policies): resolve policy components by convention` #4015 把自定义 policy 脚下的地皮整体挪了一遍，自研 policy 升级前先读这几个 PR |
| 253 | 官方 `lerobot/smolvla_libero` 自带 `n_action_steps=50`，直接吃掉约 20 个百分点 | GitHub lerobot #4614 | 2026-09-11 | Debug | [链接](https://github.com/huggingface/lerobot/issues/4614) | **本期最"便宜"的一条修复：改一个 CLI 参数换回 20pp**。Hub config 里 `n_action_steps = chunk_size = 50`，于是 `lerobot-eval` 会**把整块 50 个动作执行完才重新观测**。配对实验（同任务、同初始状态，只改这一个值）：libero_object **42/50 → 32/50（−20pp，McNemar p=0.031）**；libero_spatial **115/150 → 80/150（−23pp，p=2e-7）**。对照 SmolVLA 原论文 Table 13：50 步 51.8% vs 10 步 82.8% vs 1 步 80.3%。**`HuggingFaceVLA/smolvla_libero` 发的是 `n_action_steps: 1`，`lerobot/` 发的是 50——同名两个仓，配置相反**（又一次 #231「不要信 config」）。**最小行动：`--policy.n_action_steps=10`**。同一 issue 附带两条独立坑：(a) 0.6.1 里 checkpoint 存的 `rename_map` 必须手动重新传，**空的 CLI 默认值会覆盖存下来的那份**（修复 PR #4578 仍 open）；(b) 自 PR #4124 起 **`--env.fps` 真的会改仿真控制率且不给任何警告**——LIBERO 用 delta action，10Hz 下每个动作位移约是 20Hz 的 1.7 倍，32 个基线成功案例在不同 `--env.fps` 下的存活数为 16→3 / 18→14 / 22→23 / 25→8 / 30→1，且 robosuite 会把 substep 向下取整，`fps=30` 实际跑 **31.25Hz**。环境：lerobot 0.6.1 / hf-libero 0.1.4 / robosuite 1.4.0 / torch 2.11.0 / Apple M5 MPS |
| 254 | `PI0Policy.from_pretrained(..., strict=True)` 加载失败也返回**未训练模型**，37 个张量全零 | GitHub lerobot #4577 | 2026-09（早中旬） | Debug | [链接](https://github.com/huggingface/lerobot/issues/4577) | **本期最该贴在墙上的一条**。`strict` 确实传到了 `load_state_dict`，它也确实抛了异常——但外层 `try` 把异常吃掉、打一行日志，然后 `return model`。复现：`PI0Policy.from_pretrained("lerobot/pi05_base", strict=True)` → 只有一条 warning "Could not load state dict: ... Unexpected key(s)"，模型**照样能跑、照样输出看起来合理的动作**，但有 **37 个从未拿到权重的全零张量**（layer norm）。同一模式出现在 `modeling_pi0.py:895`、`modeling_pi05.py:1041`、`modeling_pi0_fast.py:907`。报告者自己的代价：一次 export vs original 的对比"完美一致"，因为**两边都是同一个未训练模型**，"一整批测量全部作废"。**评论里给出的 canary 值得直接抄进所有人的 eval 脚本：加载后在固定 batch 上断言输出分布与全新随机初始化的模型不同**——这一条同时能抓到部分加载。对照 #229「base ≠ 可用策略」：那是语义问题，这条是**加载器把失败伪装成成功** |
| 255 | `PreTrainedConfig.from_pretrained()` 不回填 `pretrained_path` → 归一化统计量为空的"瞎眼策略" | GitHub lerobot #4647 | 2026-09-15 | Debug | [链接](https://github.com/huggingface/lerobot/issues/4647) | **#231 / #227 这条线的第三种发作方式，而且只打程序化调用路径**。`config.json` 只带训练期字段，路径从不回填（`src/lerobot/configs/policies.py#L234-L236`）。**CLI 不受影响**（draccus 注入 `--policy.path`），**只有官方文档里的程序化用法会中招**：随后 `make_pre_post_processors(pretrained_path=None)` 跳过 safetensors、用**空的归一化统计量**新建 pipeline，于是 `NormalizerProcessorStep` 退化成恒等映射，**原始像素（0-255/255）和原始 agent 坐标（0-512 px）直接灌进网络**。实测同一个 `lerobot/diffusion_pusht` 在 PushT 上：走这条路 **cum 4.8 / max 0.016**，把 `cfg.pretrained_path` 补上后 **cum 118-199 / max 0.98-1.0**。唯一线索是**一条 `logging.warning`**。**最小行动：`from_pretrained` 之后手动 `cfg.pretrained_path = <path>`；诊断手段是 dump normalizer 的 `state_dict()` 看是不是空的**。同 issue 还报告 `lerobot.processor.migrate_policy_normalization` 当前"产出空 config.json 且不拷权重" |
| 256 | `groot` / `wall_x` 的同类静默降级，以及一份**干净清单** | GitHub lerobot #4711 | 2026-09-22 | Debug | [链接](https://github.com/huggingface/lerobot/issues/4711) | #4577 的家族审计，两个新受害者 + 一份可信名单。(1) `modeling_groot.py` 把"探测是否为微调 checkpoint"包在**裸 `except Exception: is_finetuned_checkpoint = False`** 里——于是**一次瞬时 `ConnectionError`／超时／离线模式和"文件不存在"完全无法区分**，加载器随后把你自己的微调 repo id 当成 `base_model_path` 用。附带一条性能问题：这个探测用**完整 `hf_hub_download`**（GR00T 体量是数 GB）只为判断文件是否存在，`HfApi().repo_file_exists` 一个 HEAD 就够。建议改法：收窄到 `except (HfHubHTTPError, HFValidationError, OSError)`。(2) `modeling_wall_x.py` 是 `load_state_dict(state_dict, strict=False); return model`，**完全不报 missing/unexpected key**——喂进拓扑不匹配的 checkpoint 会"返回一个半随机初始化、输出为零的模型"。**审计结论：evo1 / molmoact2 / xvla / fastwam / vla_jepa 这五个加载器是干净的**——选型时这份清单本身就是信息 |
| 257 | sync 与 RTC 推理引擎的**关节顺序不一致**：sync 一直在用"别的关节的值"驱动每个关节 | GitHub lerobot PR #4452 | 2026-09-22（merged） | Debug | [链接](https://github.com/huggingface/lerobot/pull/4452) | **本期最危险的一条，因为它会在真机上表现为"跑飞"而不是报错**。`build_dataset_frame` 按 `dataset_features[OBS_STATE]["names"]` 排 `observation.state`；`send_next_action` 却是**按位置**用 `ordered_action_keys` 贴标签，两边从来没有人校验过是否一致。PR #4416 对齐 state 之后，`SyncInferenceEngine.get_action` 里残留的 `make_robot_action` + reindex 往返就变成了**净置换**：原话是"对任何 `action_feature_names` 顺序与机器人关节枚举顺序不同的 checkpoint，**sync 会静默地用另一个关节的值去指挥每一个关节，而 RTC 驱动同一个 checkpoint 却是正确的**"。叠加相对动作（#4057）后，`AbsoluteActionsProcessorStep` 会**按位置**把缓存的 state 加回去，于是每个关节锚在另一个关节的测量值上——**在镜像对称的双臂上，目标会随着关节跟踪而朝相反方向移动，每个 tick 复合成 runaway 而不是一个固定偏移**。现场是 `bi_openarm_follower`（`left_*` 后 `right_*`）跑一个 `right_*` 在前的 checkpoint，纯粹是"旋转了 8 位"。修复：`get_action` 返回 policy 自己的维度顺序，新增 `_align_action_feature_order` 和 `_assert_state_matches_action_order` 在建 context 时就拒绝置换。**作者自己的复盘值得抄：「a happy-path-only test is what let this land」**。注：它 supersede 了 #4122——那个 PR 提的是**方向相反的错误修法** |
| 258 | `StreamingLeRobotDataset` 用**数据集全局时间戳**去查视频帧：要么崩，要么静默配错帧 | GitHub lerobot #4524 | 2026-09（PR #4528 已修） | Data | [链接](https://github.com/huggingface/lerobot/issues/4524) | 跑 `examples/training/train_with_streaming.py` + `lerobot/droid_1.0.1` 直接 `IndexError: Invalid frame index=3228251 for streamIndex=0; must be less than 149604`。根因：`streaming_dataset.py:539` 假设 `current_ts = item["index"] / self.fps` 是**文件内**时间戳，但每 episode 的 `videos/<key>/from_timestamp` 是**相对视频文件、不是相对数据集**的——在 droid 上每换一个文件就归零（ep0/ep514/ep1028/ep1541 的 from_ts 全是 0.00；fps 15，95,658 episodes，27,630,375 frames）。**静默的那一半才是要命的**：如果该视频 key 有 delta indices，值会在 `streaming_dataset.py:609` 被**夹到 episode 边界——不抛异常，但整个 batch 安静地装着错帧**。单视频文件数据集（如 `pusht` 206 episodes 一个文件）完全不受影响，这就是它长期没被发现的原因。**行动：用流式加载 + 多视频文件的大数据集（droid 这类）务必先抽样人工比对 frame 与 state/action 是否对齐**。环境：lerobot 0.6.1 / torch 2.11.0+cu128 / torchcodec 0.11.1 / hub 1.16.1 / py3.12.13 |
| 259 | `lerobot-record` 存的是**请求动作**，不是机器人实际执行的动作 | GitHub lerobot #4679 | 2026-09-18 | Data | [链接](https://github.com/huggingface/lerobot/issues/4679) | **一条会静默污染训练集的数据工程坑**。`Robot.send_action()` 的契约是返回"经限幅/安全修改后真正下发的动作"，但 `lerobot_record.py` 把它赋给 `_sent_action` 之后，**写进数据集帧的仍是先前的 `action_values`**。于是请求 `{"base.vx": 2.0}` 被硬件限到 `{"base.vx": 0.5}`，数据集里记的是 **2.0**——**你在用机器人从未执行过的指令训练策略**。发现于写 Unitree Go2 插件时。**workaround：在自己的 `send_action()` 里原地修改传入的 action dict，让 `lerobot-record` 取到限幅后的值**。Dataset v3 的悬而未决问题：`action` 该是 requested 还是 actually-sent。对照 #225（proprioception 接法）——**"记录了什么"和"发生了什么"的差距是这一类 bug 的共同形状** |
| 260 | RTC 的两处拼接缝：queue 抢跑造成**跳变**，零填充造成动作被拉向**数据集均值** | GitHub lerobot PR #4454 | 2026-09-22（merged） | Debug | [链接](https://github.com/huggingface/lerobot/pull/4454) | 承接 #15 ACTSmooth / #211 的实时分块推理这条线，这次是两个**互相独立**的 RTC bug。(1) `_check_and_resolve_delays` 即使机器人在推理期间实际消耗得更少，也照样丢弃 `real_delay` 个动作，于是 **queue 拼到了物理位姿的前面，机械臂把它当跳变执行**——修法是 clamp 到 `min(real_delay, indexes_diff)`。(2) `_normalize_prev_actions_length` 对 RTC 前缀做**零填充**，落在 guided 区域内的填充位会解码成**数据集均值**，把拼接处的动作拽向一个中性姿态——修法是保持最后一个真实动作，空前缀改为直接报错。后续还有 PR **#4717 / #4718**（"Fix/rtc chunking seams 2 / 3"）。**读法：抖动/跳变不一定是策略学得差，先排查推理引擎的缝** |
| 261 | 四条"不报错"的小坑：`use_tanh_squash` 空操作、DAgger 录不满一集、`repo_id` 裸名崩溃、数据集不声明关节语义 | GitHub lerobot | 2026-09 | Debug | [链接](https://github.com/huggingface/lerobot/issues/4727) | 打包四条本期 issue，共同点都是**契约与实现不一致**。(1) **#4727**（09-23）：`policy_kwargs.use_tanh_squash` 一路传到 `modeling_gaussian_actor.py:411`、在 `:421` 被存下来，**然后再也没人读它**——`forward` 无条件构造 `TanhMultivariateNormalDiag`。512 样本 × 观测放大 50× 实测：True → max|action| 0.9987，False → 0.9975，**两边都没超出 [-1,1]**，纯静默空操作。同文件另一处气味：`Policy.__init__` 默认 `std_min=-5, std_max=2` 而 `PolicyConfig` 是 `1e-5, 10`。(2) **#4626**（09-13）：`DAggerStrategyConfig.record_autonomous=True` 走 `_run_continuous()`，episode 边界由**估算文件大小**决定；`False` 走 `_run_corrections_only()`，每个遥操窗口算一集。**没有任何模式能录"一次完整任务尝试（自主 + 干预、内联标记）"**——而这恰好是 DAgger / HG-DAgger / RaC 真正需要的；报告者"从来没能录出超过一集"。(3) **#4663**（09-18，Windows 11 + LeLab 0.1.0 + 未登录 HF）：`sanity_check_dataset_name` 对不含 `/` 的裸数据集名做 `repo_id.split("/")` 直接 `ValueError`（修复 PR #4674）。(4) **#4749**（09-24）：`meta/info.json` **既没有办法通用地识别夹爪通道，也不声明关节空间还是任务空间**——按 `gripper`/`grip`/`jaw` 名字匹配是猜；在 `pusht`（根本没夹爪）上"最后一维是夹爪"的兜底会挑中 agent 的 y 坐标，"给出一个自信而毫无意义的结果"。呼应 #4749 提的 per-feature `role` 注解请求 |
| 262 | Figure Helix 2.5：**30 户没见过的真实住宅零样本**，以及 9% → 56% 的受控消融 | Figure AI | 2026-09-17 | Recipe, Strategy | [链接](https://www.figure.ai/news/helix-2-5-zero-shot-30-home-generalization) | **本期数字密度最高的一条，而且是少见的"一切其他变量固定"的预训练消融**。3 个全身行为（客厅收拾、叠毛巾、铺床）在 **30 户湾区未见过的住宅**里零样本运行：这些家里没采过一帧数据、没微调、每个任务**一个固定 checkpoint 跑完全部 30 户**。**消融（架构/优化器/超参/下游数据/评测全部固定，只换初始化）：随机初始化 9% vs Index 预训练 56%——6× 差距**。评分是**全有或全无**：13-15 个玩具全部入筐 / 毛巾全部叠好入筐 / 两个枕头 + 被子两角都在上三分之一且铺平。**架构上的方向反转值得记**：Helix 2.5 **从随机初始化、完全在 Index 上预训练**，而 Helix 02 是从预训练 VLM 起步的——与 #14 Helix / #188 Helix-02 连读。另外两个数字：用**一半的任务特定适配数据**追平 Helix 02 的成功率，同时泛化范围宽 30 倍；**人→人形迁移的 scaling law** 在 8× 预训练数据范围上、模型大小与下游训练固定的 4 个模型间，**把最大那次 run 的留出动作预测 loss 提前预测到小数点后 4 位，误差是 8× 范围内变化量的 0.54%**。**评测协议值得直接抄**：逐项超时（1 分钟/玩具、3 分钟/毛巾、1 分钟/枕头与被子每边），折叠质量 A/B/C 分级（四角误差 1 英寸内算 A），**任何安全干预 = 直接判失败**；评测物品在实验前就被隔离，并由 AI 模型 + 人工双重核验其不在任务数据中。单个评测任务在 Index 预训练混合中占比不超过 **1.90%**。Index 吞吐"每秒约 35 分钟新的人类经验"，训练算力承诺 **$3.5B**（对应 09-03 与 Nscale 的 Vera Rubin 平台、最多 10 万卡协议） |
| 263 | Skild AI：**"99.9% 准确但慢 10 倍不是差一点能部署，是不能部署"** | Skild AI | 2026-09-10 | Strategy | [链接](https://www.skild.ai/blogs/skild-crosses-100m-arr) | 名义上是营收帖，但含本窗口**最有用的一条部署约束**，直接上引号：一条有 10 个工位的线必须节拍匹配，否则最慢那个工位封顶整线吞吐——**所以"限时成功率"必须从第一天就是指标，而不是先做准再做快**（与 #208 Figure 03 的 84s 节拍、#19x 系列的吞吐观点同源，但这是第一次被说成硬性选型原则）。**最值得记的工程数字是"最后 5% 的代价"**：煮蛋**第一次成功用了 1 周，做到跨不同鸡蛋与不同布置都可靠用了 2 个月**——他们的说法是挤最后 5% 的工作量超过前 95%。商业侧数字（对判断产业成熟度有用）：**$100M ARR、已确认 $50M、60+ 付费客户**，距首次商业部署 10 个月；**约 90% 营收来自操作、10% 移动**（其中 4% 是 Zebra/Fetch 并购来的 AMR）。具名部署：NVIDIA + 富士康 Blackwell 系统组装的双臂、**住友电装**线束制造、**三井物产**商用厨房（其供应链日供 140 万份餐）。他们把 S1 的 in-context learning 直接说成是对**部署端变更频率**（供应商换件、工厂挪工位）的回应——因为逐次变更都做 post-training 不可规模化 |
| 264 | Skild Physical Self-Play：**140 年**模拟对打，涌现带球/护球/铲球/摔倒起身——但全程无数字 | Skild AI | 2026-09-23 | Arch | [链接](https://www.skild.ai/blogs/physical-self-play) | 在 NVIDIA Isaac Sim 里对 S1 做自博弈后训练：**单一目标函数（得分）**，对手从自己最近的版本中抽取，累计 **140 年模拟对打**，然后 sim-to-real 迁到人形上打真实比赛。带球、护球、铲球、摔倒后起身**都没有手写奖励**。**但这条必须打上折扣标签：全文定性，没有成功率、没有 sim-to-real gap 数字、没有算力数字**——作为"自博弈能否替代奖励工程"的**方向信号**收录，不作为可复现结果。承诺下一篇讲多智能体社会行为。对照 #223 τ₀-VLA（world-model 引导的 test-time 搜索）：两者都在赌"不靠人标的经验来源" |
| 265 | FLUX 3 Action：**7B 开源 world-action model**，以及"用电子游戏当评测加速器"这个骚操作 | HF Blog (Black Forest Labs) | 2026-09-23 | Arch, Recipe | [链接](https://huggingface.co/blog/black-forest-labs/flux-3-action) | **本期唯一一条真正的 HF Blog 内容，也是 WAM 路线（#198 #202 #203）第一次有主流开源权重**。7B diffusion transformer，冻结视频 VAE + 冻结 **Qwen3-VL-4B** 文本编码器；**动作是同一个序列里的第二条 token 流**，每个未来帧一个 token，逐本体的输入投影与输出头，**视频与动作 token 共享同一噪声水平、联合去噪**。输出 **32 个动作（约 2 秒）**，可选 32 个解码帧——**控制时跳过解码**，执行前几个动作、重新观测、重规划。输入几何很具体：3 路 DROID 相机 **544×736**，或 2 路 SO-101 并排，或单帧 512×512，全部拼到一张 VAE 画布上，外加本体自己空间的 state 向量。**RoboLab-120 排行（这张表本身就是本期最有信息量的横向对比）**：FLUX 3 Action **42.92%** > OASIS WAM 39.0% > Cosmos3-Nano-Policy(16B) 36.8% > Phoenix 34.4% > BiMind v0.1 33.3% > π0.5 28.0% > **DreamZero(14B) 25.7%** > Cosmos3-Edge-Policy(4B) 22.9% > π0-FAST 15.5% > GR00T N1.6 7.2% > π0 5.0%。真机：SO-101 上仅 **~200 条遥操 episode** 的相关抓放，泛化到未见物体、未见容器、**被笔记本遮挡 60% 的容器**、以及**训练与 rollout 之间改变了相机位置**，并能从自己的失误中恢复。**最值得抄的方法论是那个评测加速 hack**——他们明说"在硬件上评一个 checkpoint 可能要一整个下午"，于是用两个自研游戏（各 **800 episode**、由只看画面的脚本 bot 产生）当快速评测台：GRUNT（类 Quake，256×256，4 动作）FLUX 15 杀 / 0 死 / 86% 命中 vs bot 13 / 0 / 96%；VECTOR（赛车，3 动作）60 秒 3.3km / 1 次撞毁 / 12 次超车 vs bot 3.4 / 0 / 16。**一套权重跑两个游戏，靠 caption 区分**。室内无人机：256×256 机载相机、[前进/横移/上升/偏航]，**仅用 Isaac Sim 里 800 次脚本飞行做微调**（基座模型没见过仿真数据），泛化到未见房间与改写指令。训练在 **NVIDIA GB200** 上、自定义 **CuTe DSL** kernel；PEFT 配方与 Jetson 边缘部署与 NVIDIA 合作；已有原生 LeRobot policy 类。License 是 FLUX Kommunity License v1.0（**不是 Apache-2.0，商用前先读**） |
| 266 | Isaac ROS 5.0 的 `rosidl::Buffer` CUDA 零拷贝：省掉自定义消息，但有**四个静默回退条件** | NVIDIA Developer Blog | 2026-09-22 | Edge | [链接](https://developer.nvidia.com/blog/accelerating-a-ros-2-node-with-an-ai-agent-and-nvidia-isaac-ros/) | 接 #4368（LeRobot × ROS 2 的官方方向讨论）这条线，产品化路径上的实际管道工作。**`rosidl::Buffer` + NVIDIA 贡献的 CUDA buffer backend 已上游进 ROS 2 Lyrical**，Isaac ROS 5.0 全部节点迁过去了：标准 `sensor_msgs/msg/Image` 就能走 GPU 常驻零拷贝，**不需要自定义消息类型、不需要并行的 CUDA topic**。**快速路径的硬前提（不满足就静默回退到 CPU）：同一主机、同一 CUDA 设备、同一 Linux 用户、以及受支持的 RMW（`rmw_fastrtps_cpp` 或 `rmw_zenoh_cpp`）**——底层是 CUDA Virtual Memory Management。迁移面比想象小：一个 `SubscriptionOptions.acceptable_buffer_backends = "cuda"`、一次 `cuda_buffer_backend::allocate_buffer()`、两处 stream-aware 句柄提取（`from_input_buffer` / `from_output_buffer`）、一次 publish；依赖加 `cuda_buffer` + `cuda_buffer_backend`。**一个会安静出错的坑：写句柄必须在 publish 之前的内层作用域里释放**，否则写 CUDA event 没被记录、顺序保证失效。验证配方：**Nsight Systems 确认 ROS 边界上不再有 payload 尺寸的 H2D/D2H**，再加运行时断言 `msg->data.get_backend_type() == "cuda"`，并做 CPU 源 / CUDA 源双管线测试。**这篇的缺口要标出来：全文没有任何延迟或吞吐数字**，作者让你自己"记录迁移前后可比的延迟测量"。示例是 Depth Anything 3 的 TensorRT ROS 2 节点，目标平台 Jetson AGX Thor；配套 agent skill `migrate-node-to-rosidl-buffer` 在 isaac-ros-cli |
| 267 | Isaac-GR00T 官方部署文档里的六条硬约束 + 全平台 TensorRT 实测延迟 | GitHub NVIDIA/Isaac-GR00T | 2026-09（文档当前状态） | Edge | [链接](https://github.com/NVIDIA/Isaac-GR00T) | **边缘部署 GR00T 之前必须知道的东西，全部来自官方 `scripts/deployment/README.md`，不是二手总结**。(1) **Orin 上 TRT 10.3 不支持 backbone（LLM）引擎**——必须 `--export-mode dit_only` 构建、`--inference-mode tensorrt` 运行（DiT 走 TRT，backbone 留在 PyTorch）；完整管线只在 dGPU / Thor / Spark 上是推荐路径。(2) **`--batch-size` 被烧成 ONNX 与 TRT 的静态维度**，换 batch size 必须重跑 `--steps export,build,verify`。(3) **引擎与 GPU 架构绑定**，换卡必须重建（约 2-5 分钟）。(4) ONNX 导出**显式使用 legacy exporter（`dynamo=False`）**——原因是"dynamo exporter 会把 `vl_seq_len` 这类动态序列维度特化掉，破坏 TRT 对不同 token 长度的运行时灵活性"。(5) **aarch64（Spark/Thor/Orin）上跑完 `install_deps.sh` 后绝对不要用 `uv run python` / `uv run torchrun`**——"后者会对着根 `pyproject.toml`（x86_64 / Python 3.12）重新 sync，摧毁平台特定环境"；正确做法是 `source .venv/bin/activate && source scripts/activate_<platform>.sh` 然后用普通 `python`。(6) TRT 评测需要 `--n-envs 1`（"ViT 引擎对单观测推理是静态形状"）。**排障对照表值得贴墙**：COMPLEX128 导出错误 → 确认用的是 `_simple_causal_mask` 而不是 HF 的 `create_causal_mask`；`masked_scatter` 尺寸断言 → `visual_pos_masks` 的 True 数量必须与 deepstack 张量尺寸一致；cosine < 0.99 → 检查 LLM 导出是否排除了最后的 RMSNorm；**"输出幅值小了约 12 倍：这就是 norm bug"**。**实测端到端（4 步去噪、1 相机）**：H100 85.8ms/11.7Hz eager → 48.6ms/20.6Hz torch.compile → **27.9ms/35.9Hz TRT 全管线（3.08×）**；Orin 342.8ms/2.9Hz → 216.5ms/4.6Hz（DiT-only，1.58×）；AGX Thor 144.9 → 93.8ms（10.7Hz）；DGX Spark 126.4 → 98.6ms（10.1Hz）。**代价要一起记：LIBERO 闭环 20 episode，PyTorch 100% vs TRT 95%**——加速不是免费的（与 #269 同一教训）。另：所有推理/微调命令都要 `--embodiment-tag`，它选的是 modality config，**其中包含归一化** |
| 268 | 核查：**GR00T N2 至今未发布**，"RoboArena 第一"是 3 月的说法且已被本期数据打问号 | NVIDIA Newsroom | 2026-03-16（核查于 2026-09-25） | Strategy | [链接](https://nvidianews.nvidia.com/news/nvidia-and-global-robotics-leaders-take-physical-ai-to-the-real-world) | **一条纠偏项，因为这四句话正在被当成"新消息"到处转**。原文逐字核实无误：GTC keynote 上"预览"了基于 **DreamZero** 研究的 GR00T N2，"建立在新的 world action model 架构上"，"在新环境的新任务上成功率是领先 VLA 模型的**两倍以上**"，"**预计年底可用**"，"目前在 MolmoSpaces 与 RoboArena 的通用机器人策略上排第一"。**但**：(1) 这是 **2026-03-16 的 keynote 预览，不是本窗口的新闻，且 N2 仍未发布**；GTC 实际放出早期访问的是 **GR00T N1.7-3B**（#213）。(2) **"第一"是 3 月的 "currently"，现在很可能已过期**——本期 #265 的 RoboLab-120 表里 DreamZero（N2 的研究基座）只有 **25.7%**，低于 Cosmos3-Nano-Policy 36.8% 与 FLUX 42.92%；不同 benchmark 不构成直接反驳，但**不支持"仍然第一"的说法**。(3) **">2×" 在新闻稿里没有任何方法论**——没有任务表、没有具名基线、没有 n。DreamZero 的技术底：NVIDIA 自家 WAM 综述（2026-06-15）给的是 **Wan 2.1-I2V-14B 骨干、2026-04 RoboArena 榜 1750 Elo vs π0.5 1622**，动作调优约 **9 ZFLOPs**、完整视频预训练 + 适配可超 **50 ZFLOPs**（arXiv 2602.15922）。**读法：把"预览 + 榜单第一"当营销时间戳，不当选型依据**（延续 #207 的评测公信力问题） |
| 269 | SmolVLA 转 ONNX：p99 延迟砍到一半、Spatial 掉 29pp——**而真凶是 16-token 的语言宽度截断，不是精度** | arXiv 2609.14146 | 2026-09-12 | Debug, Edge | [链接](https://arxiv.org/abs/2609.14146) | **本期最有教学价值的一条，而且它对自己的标题做了反驳**。论点：三个常被当成等价的量其实不等价——(i) 共享输入上的开环数值一致性、(ii) 某张卡上的延迟/显存、(iii) 闭环成功率（每个动作都改变下一个状态，微小数值漂移会复合）。设置罕见地可复现：`HuggingFaceVLA/smolvla_libero`、**RTX 2060（6GB）**、LIBERO Spatial 与 Object、MuJoCo 3.3.2、LeRobot 0.6.1、seed 42。**数字**：PyTorch+AMP **70.0% / 88.0%（Spatial/Object），p99 1181ms**；请求 FP16 的 ONNX CUDA EP **p99 601ms 但 Spatial 掉到 41.0%**；请求 INT8 **532ms、Spatial 40.0%**；**Object 全程稳在 89.0%**。**三条必须一起记的更正（本条已交叉核对论文正文）**：(1) **那个"INT8"不是 INT8**——论文自己的 graph audit 说两个产物"是逐字节相同的 FP32 图"，532ms 只是同一张 FP32 图的一次重复，601 vs 532 的差是 CUDA-EP 运行时抖动。**任何把"INT8=532ms"当量化结果引用的人都在反着引用这篇**。(2) **Spatial 的崩塌主要是接口 artifact 而非精度/延迟效应**——导出器默认 16 token 的语言宽度**截断了 10 条 Spatial 指令中的 5 条**；按宽度 24 重新导出，Spatial **回到 75.0% 而延迟几乎不变（p99 583.6 → 587.7ms）**。(3) PyTorch 基线自身（70.0/88.0）就远低于 SmolVLA 论文的 96.0/92.0。**仅仿真，作者明说"没有用 Jetson、没有用物理机器人"**。**读者行动：导出/量化后必须跑闭环，而且第一件事是检查 tokenizer 宽度有没有把你的指令切掉**——这比精度问题常见得多也便宜得多 |
| 270 | VLAQuantBench：**把 π0.5 action head 里更多的层量化掉，成功率从 7.0% 涨到 70.5%** | arXiv 2609.25376 | 2026-09-21 | Debug, Edge | [链接](https://arxiv.org/abs/2609.25376) | 反直觉到需要反复读一遍的一条。论点：VLA 的 PTQ 精度选择**不能按格式一刀切**，取决于层范围 × 数值格式 × 校准三者的交互。规模：**409 次运行、94,574 个仿真 episode**，4 个模型在 LIBERO 上，X-VLA 另加三个 benchmark 家族（SIMPLER / CALVIN / VLABench）。**关键数字**：在未校准的 W4A4 round-to-nearest 下，把 π0.5 的 **action-head 被量化层集合从 126 层扩到 167 层，LIBERO Spatial 成功率从 7.0% 升到 70.5%**——**多量化 41 层换来 +63.5pp**。**两条不可省的限定**（本条已交叉核对，初稿曾把方向写反）：**126→167 是"被量化"的层数在增加，不是"保高精度"的层数在增加**；而且 **70.5% 仍比未量化基线低 28.5pp**，这是 LIBERO Spatial 的单项数字不是均值。**仅仿真**。**读法：这条与 #3（量化 flow matching action expert 严重降精度）不矛盾——它说的是"哪些层"这个维度上存在非单调性，所以边缘部署的量化范围必须闭环扫，不能按"action head 敏感、保精度"的直觉一把梭**。代码：github.com/jiuyixu25/VLAQuantBench |
| 271 | EffVLA：action head 的性能**主要由初始化决定**，不是 decoder 架构、不是 loss、不是推理预算 | arXiv 2609.13984 | 2026-09-12 | Arch | [链接](https://arxiv.org/abs/2609.13984) | **如果这条成立，它会作废相当多的 action expert 设计工作量**，所以值得当成一个待证伪的强命题收着。方法难得规范：固定骨干家族（SigLIP2、Qwen2.5）与训练流程，只扫 action head 设计与模块规模，**每个配置都配一次实测的端上延迟**。三条结论近乎逐字：(1) action head 性能"主要由初始化而非 decoder 架构、loss 或推理预算决定"；(2) **把语言骨干的最后若干 transformer 层直接拷进 head 是单一最大杠杆，零延迟代价，而且是唯一在所有模块规模上都有效的轴**；(3) **flow matching 与更重的 decoder 只在 head 尚未对齐时有收益，一旦对齐反而变成负收益**，额外推理次数没有可测收益。验证：LIBERO + LIBERO-Plus 仿真，**外加真机 SO-ARM101（配方不变直接迁）**——单臂低成本平台，真机证据偏弱但不是纯仿真。**与 #13（L1 regression 在不完美数据上比 diffusion 更鲁棒）连读：两条从不同方向指向同一件事——动作生成器的表达力不是瓶颈，对齐才是** |
| 272 | FailBench：**拿 VLM 当成功检测器，在接触密集任务上不超过 0.60 balanced accuracy**；而且专门微调的更差 | arXiv 2609.03611 | 2026-09-03 | Debug | [链接](https://arxiv.org/abs/2609.03611) | 直接打在 #209（Robometer / TOPReward 把奖励模型升为一等 API）这条线上——**在把 VLM 判分接进 RL 或数据筛选之前先读这个**。Benchmark：**2,197 次操作尝试、14 个公开来源（12 个真机 + 2 个仿真）、75% 的失败是自然发生的**而非脚本注入，评了 13 个检测器。**数字**：最好的检测器只到 **0.77 平均 balanced accuracy**；**当成功与否取决于可观测的物体位移时接近饱和，而当成功取决于"是否建立接触"时接近随机——接触密集的装配任务上没有任何模型超过 0.60**。两条附加诊断同样要紧：**为失败检测专门微调过的模型持续不如通用 VLM、甚至不如它们自己的预训练基线**；以及仅仅把画面裁到与结果相关的区域，就能让最强检测器 **+2.3 点**、无需重训。**性质要标清：这是离线判分 benchmark（VLM 判录好的尝试），不是闭环部署**。配套读 **arXiv 2609.09250「No Free Checker」**（约 150 个验证器的综述，把"可得性"与"可信度"摆成互相拉扯的两个轴）——那篇给框架，这篇给经验证据 |
| 273 | GE-Act 2.0：**共训数据从 300 小时加到 30,000 小时（100×），成功率只从 17.1% 到 44.1%** | arXiv 2609.05588 | 2026-09-04 | Data, Strategy | [链接](https://arxiv.org/abs/2609.05588) | **本期最该被放进任何"要不要继续堆数据"讨论里的一条**。论点：几乎所有 WAM 都继承一个预训练视频生成器，所以 **WAM 自己的预训练与 scaling 从来没被真正研究过**；GE-Act 2.0（AgiBot / Genie Envisioner 线）把**每一个可训练的生成与动作组件都从零在操作数据上初始化**。组件：control-oriented autoencoder（CoAE）、single-step visual planner（SVP）、inverse dynamics model（IDM）；配方是**用无动作视频预训练 planner、用无语言轨迹预训练 IDM，再用全标注演示共训整个系统**——这个"分模态预训练再合"的结构本身值得抄。**Scaling 数字**：共训池 **300 / 1,200 / 5,000 / 30,000 小时**，G1-OP 100 任务真机套件零样本成功率 **17.1% → 44.1%**，G2-90D **13.4% → 31.1%**；20 个技能组里 19 个有提升，**有非零成功率的任务数 39 → 76**；覆盖度与 OOD 成功的相关性 **Pearson r=0.80 / Spearman ρ=0.85**。**真机验证**（G1-OP / G2-90D 为物理本体，100 任务、留出场景/背景/光照/物体实例、零样本不做逐任务微调）。**读法：100× 数据买到 2.6× 成功率，而且最后仍有 24 个任务是零——这是"数据飞轮有效但远不够"的最干净一次量化**（对照 #213 #220 #223 的 3-4 万小时门槛：门槛是真的，但门槛之上的边际收益很薄） |
| 274 | 无 GPU / 省算力两条路线：`vla.simd` 的 CPU 推理（含 **90 秒热浸泡**）与 rMuscle 的内部状态缓存 | arXiv 2609.24274 / 2609.19104 | 2026-09-21 / 2026-09-17 | Edge | [链接](https://arxiv.org/abs/2609.24274) | 接 #203 vla.cpp / #204 Embodied.cpp 这条边缘运行时线，同一批作者（VinRobotics）的下一步。**`vla.simd`**（2609.24274）：共享 SIMD 微内核 + 可复用计算 + 目标特定优化，**跨 6 个 policy × 4 种 CPU 约 1.4× 中位加速，保持 fp32 数值保真**。同时提出 IMPACT（ACT 基座 + 缓存文本表示 + 语言调制视觉特征），报告它是**其评测集中唯一能在 Raspberry Pi 5 上供出 ≥30 actions/s 的语言条件策略：90 秒热浸泡之后 fp32 33.5 actions/s、int8 81.2**。**"90 秒热浸泡"这个细节几乎没人报，但它是嵌入式平台上唯一诚实的测法**——建议直接抄进自己的边缘 benchmark。真机：IMPACT 在 SO-101 上（开抽屉 / 放胶带 / 关抽屉），另有 vla.simd 从 M4 与 Ryzen 主机服务 SmolVLA 驱动 UR10e + Robotiq，两台主机各 12/20 成功。**注意 IMPACT 是作者自研策略，Pi 5 那个数字是协同设计的最好情况，不是"现有 VLA 能在 Pi 5 上跑"**。**rMuscle**（2609.19104）：论点是重复执行（例如工厂固定工位）不只观测与轨迹自相似，**内部模型状态也自相似**，所以可以缓存——双缓存设计削 VLM 计算与去噪权重搬运，加在线缓存重建、滑窗缓存检索、跨去噪步共享 mask；**RTX 4090 与 Jetson Thor 上端到端 1.29-1.42× 加速，仿真与真机成功率均保持**。**两条路线的共同前提都是"任务重复"**——一次性任务上收益归零 |
| 275 | 数据侧两条可直接上手的：RoboDrop 用梯度相容性筛掉坏演示（35.0% → 63.75%），TraceFlow 把**自己的失败 rollout**变成指引 | arXiv 2609.10021 / 2609.20646 | 2026-09 | Data, Recipe | [链接](https://arxiv.org/abs/2609.10021) | 接 #230-#233 的"数据质量比数量重要"这条线，本期第一次给出**可自动化的筛选与利用手段**。**RoboDrop**（2609.10021，已核对正文 Table 5）：坏的 post-training 演示可以靠"每个样本的局部梯度是否与一个上下文条件参考方向相容"来检测——参考方向由任务语义与视觉上下文匹配的干净验证样本构造。测了三种坏监督：破坏观测-动作对应的合成损坏、低技能仿真操作员的邋遢演示、真机里的非专家执行失误。**数字（4 个真机任务 × 20 次试验）：不筛 35.0% → 固定删 20% 的 RoboDrop 63.75% → 自动预算 67.5%**；最强基线 Behavior Retrieval 58.75%、DataMIL 48.75%、QoQ 46.25%、Scizor 45.0%；单任务上 HANG CUP **2/20 → 9/20**。**真机验证**。（标注一处：摘要写"35.0% 到 67.5%"，正文 RoboDrop 主行是 63.75%，67.5% 是自动预算变体。）**TraceFlow**（2609.20646）：值得注意的是它**只用终局成功/失败一个 bit** 就能把机器人自己的失败 rollout 当负面证据用——不需要 critic、不需要 verifier、不需要动力学模型、**不更新任何权重**。做法是把带终局标签的时序 state-action 记录攒成 TraceBank（先用目标任务训练轨迹播种，之后纳入部署 rollout），构成一个与进度对齐的指引场，对冻结的 flow matching action expert 做有界修正。**数字——真机有序装箱任务：基座 50 次里 21 次按序完成，TraceFlow 39/50，再攒一轮（零权重更新）47/50，顺序错误的 episode 从 20 降到 0**；仿真 RoboMemArena Sequence 78.92% → 91.50%。**它对代价很诚实：26 任务的总体聚合没变，Counting 与 Occlusion 分别降了 1.12 与 1.42 点**——这是**定向增益而非普遍增益**，与 #251 的读法一致 |

---

## 本期 arXiv 速查（2026-08-28 → 2026-09-25 窗口，仅核对到摘要）

> **纪律声明（沿用 #225 / v3.12 的标注惯例）**：本表全部数字**取自 arXiv 摘要或第三方检索摘要，未展开正文交叉验证**；#269-#275 那几条是本期唯一做过正文/全文核对的（并因此改掉了两处方向性错误，见 #269 #270）。投稿或对外引用前请自行打开原文核对。
> **本期额外的工具限制**：`web_fetch` 对 arXiv `abs` 页在若干次成功后开始返回 `application/pdf`（无可读文本）并触发 HTTP 429；`export.arxiv.org` API 与 `arxiv.org/list/` 均因 provenance 限制无法直接取。**可用的正文验证通道是 `arxiv.org/html/<ID>`**——下期直接走这条。

| # | 标题 | arXiv | 日期 | 类别 | 一句话 + 关键数字 | 验证等级 |
|---|------|-------|------|------|------------------|---------|
| A1 | H-VLA：把「下一个子目标」与「稠密动作」拆成两个模型，统一在相机中心动作空间 | [2609.22895](https://arxiv.org/abs/2609.22895) | 09-19 | Arch | SimplerEnv Google Robot visual matching 91% / variant aggregation 84% / WidowX 81%；**Agilex 真机：同分布 +10pt，OOD 位置 +47pt，OOD 场景/物体 +16pt**。**47pt 这个差值本身是信号——同分布数字几乎掩盖了架构的全部价值**（呼应 #251 #215） | 真机 + 仿真 |
| A2 | Sim-to-Real 集成管线：把仿真专家轨迹**开环重放在真 Franka FR3 上**采数据，跳过遥操 | [2609.21817](https://arxiv.org/abs/2609.21817) | 09-18 | Recipe | IROS 2026 Sim2Real workshop。因为每条真机录制都与产生它的仿真轨迹一一配对，**这个协议给出的是逐轨迹的 sim-to-real gap 直接测量**，而不是间接的成功率差。摘要无头条成功率——交付物是协议 + 已公开的数据集（HF）与代码（gitlab.isir.upmc.fr） | 真机 |
| A3 | LIBERO-VPro：在**执行中**扰动视觉证据，含几乎没人测的「相机陈旧（Camera Staleness）」轴 | [2609.24350](https://arxiv.org/abs/2609.24350) | 09 | Debug | 四轴（视觉证据退化 / 相机陈旧 / 视觉源一致性 / 任务相关场景变化）、12 类挑战、96 个设置、3,296 个任务-条件组合；3 个 VLA + 3 个 WAM，**约 196,000 个仿真 episode + Franka Research 3 上 200 次真机 rollout**。**相机陈旧是部署里最常见、benchmark 里最少测的一轴** | 仿真 + 真机 |
| A4 | IndustrialVLA-Bench：干净 benchmark 分数**几乎不含区分信息**，改写指令才含 | [2609.25562](https://arxiv.org/abs/2609.25562) | 09 | Debug | 固定 checkpoint 与推理配置、三次不同种子的完整评测。**6 个系统间：干净 LIBERO 均值只差 1.58 分，鲁棒性（LIBERO-Plus）跨度 14.62 分，改写指令（LIBERO-Para）跨度 31.08 分**；只看三个协议忠实的系统仍是 1.36 / 14.62 / 23.10。**31 分的改写跨度意味着多数 LIBERO 榜单差值是噪声** | 仅仿真 |
| A5 | Coda：**增加 flow matching 积分步数抬高推理成本，但不必然改善闭环成功率** | [2609.21216](https://arxiv.org/abs/2609.21216) | 09-18 | Debug, Arch | 把一部分积分预算改配给一次学出来的终点修正：冻结策略跑几步 noise→action，再用轻量 Transformer 从候选动作、源噪声与共享的观测 prefix cache 预测一个演示监督的残差。**未取到定量结果——可引用的部分只有"加步数没用"这个否证**。与 #271「额外推理次数没有可测收益」独立指向同一结论 | 未确认 |
| A6 | TacPAC：**把未来触觉当成"另一路视角"去预测，只拿回约三分之一的可得收益** | [2609.05266](https://arxiv.org/abs/2609.05266) | 09-04 | Debug, Arch | 原因是**预测发生在执行之前，而触觉反馈在执行之中才到**。做法：基座规划完一块动作后，缓存它所依据的预测接触与该规划的表示；触觉专家拿每一张新观测到的触觉图去对这份缓存，修正**尚未执行**的动作。**一次修正是对缓存的单次前向，比重新生成整块动作便宜 20.7×；5 个真机任务平均成功率 22% → 64%**，每个任务都领先。承接 #239 TacForcing | 真机 |
| A7 | ZETA：**"zero-shot 跨本体"这个词在文献里没有统一定义，结果不可比** | [2609.02546](https://arxiv.org/abs/2609.02546) | 09-02 | Debug, Strategy | 提出**严格 zero-shot（目标本体在全部训练数据中缺席）vs 预训练已见 zero-shot（目标本体只出现在预训练里）**的区分——**这个区分一直在静默抬高已发表的数字**。14 个留出目标本体，隔离四个因素：状态-动作表示、预训练本体多样性、辅助共训目标、目标本体暴露程度。未取到逐因素数字。与 #217 The Embodiment Gap 连读 | 仿真 + 真机验证 |
| A8 | FARM：失败信号**已经线性可解码地存在于冻结世界模型的预测状态里**，不需要单独的检测模型 | [2609.11445](https://arxiv.org/abs/2609.11445) | 09-10 | Debug, Edge | 只训一个 **33,985 参数**的监督读出头，挂在冻结的 VLA-JEPA 预测状态上，输出逐步失败分与因果轨迹风险。7 个源任务五折 out-of-fold **AUROC 85.68 / AUPRC 88.59**；在 10 任务 benchmark 上于 **15 个对照基线**中 Seen 表现最好；**拿到冻结状态后平均 CUDA 延迟 0.2256ms**。跨 PIPER X / SO-101 / Franka 四个真机群体，含固定读出头迁移与只调读出头的适配 | 真机 |
| A9 | ActiveScale：给 VLA 加历史视频 + **显式相机位姿监督**，解决固定相机把关键结构遮住的场景 | [2609.18514](https://arxiv.org/abs/2609.18514) | 09-16 | Recipe, Data | 逐帧 pose token + 轻量预测头，并在人类活动中天然存在的相机运动上做中训。**配方数字：人-机中训阶段用 1,000 小时数据**。**结果：相对直接后训的 π0.5，平均 SR 30.0% → 70.0%、平均 TP 41.6% → 78.4%**。与 #230（真机相机两分钟漂 37px）/ #215（GS-VLA）同属"相机"这条主线，但走的是"学会跨视角推理"而不是"把视角标准化回去" | 真机范围未确认 |
| A10 | VLA-Precision：真机在线 RL 后训练，**平均每个任务 45.8 分钟**训到 98.3% | [2609.04355](https://arxiv.org/abs/2609.04355) | 09（初） | Recipe | 非对称协同自举（ACoB）跨时间尺度：早期干预引导的行为学习既快速改进策略、又同时改进在线经验质量，配 ACoB-Stream 架构。**9 个高精度化学任务、4 类、4 种本体：平均成功率 98.3%，平均每任务 45.8 分钟在线训练，成功试验平均耗时 27.6 秒；相对最强真机 RL 基线（Robo-Dopamine）平均成功率 +88.3%、平均执行速度 +61.2%**。**"45.8 分钟/任务"是这条里最可用的工程数字**（对照 #20 HIL-SERL 的 1-2 小时） | 真机 |
| A11 | Imagine-RL：冻结世界模型做**视觉-力矩**想象，残差当置信度先验 | [2609.24033](https://arxiv.org/abs/2609.24033) | 09 | Recipe | VLA 与 VTLWM 都冻结、只动 critic/actor；前一窗口的预测残差变成 token 级置信度先验，抑制不可靠的未来 token。**4 个真机任务 × 50 次评测，只用 100 条 RL 轨迹，相对 DSRL +23.6%、相对 VLA 基线 +60%**。**100 条轨迹的预算是可用的部分** | 真机 |
| A12 | RAPolicy：异步 rollout + **replay 锚定**的真机在线后训练，critic 不预测下一个动作 | [2609.22888](https://arxiv.org/abs/2609.22888) | 09-19 | Recipe | critic 从记录下来的动作学 chunk 级价值并**在不预测下一动作的前提下**构 Bellman 目标，避开 replay 覆盖之外的动作价值估计；one-step flow actor 复用 rollout 期存下的初始噪声，按优势加权条件似然学习——**监督的正是执行时真正用到的那个映射**。协议：4 个单任务 + 1 个五任务联合，**在线训练预算仅 1-2 小时**，单任务 20 试/任务、联合 10 试/任务。**成功率只写了"更高"，未取到具体数字** | 真机 |
| A13 | ForeTac-VLA：预测**多步未来触觉状态**，让策略对已观测与预期接触一起推理 | [2609.20980](https://arxiv.org/abs/2609.20980) | 09 | Arch | 触觉时序表示 + 双向 cross-attention 融合视觉语言特征 + transformer 预测模块。**4 个真机接触密集任务平均成功率 95%，比微调后的 VLA +36.25pt、比 SOTA 触觉增强 VLA 基线 +22pt 以上**，且**在低照度与视觉杂乱下保持**——正好是 A3 LIBERO-VPro 所测的那些视觉失效模式，**触觉作为视觉退化的对冲**这条论证第一次有了配对证据 | 真机 |
| A14 | HapticWAM：**推理时不用触觉传感器**，把"想象的触觉"蒸进世界-动作模型 | [2609.23888](https://arxiv.org/abs/2609.23888) | 09-22/23 | Arch | 投 ICRA 2027。教师把 gel 图像与形变、剪切、分布力、合成 wrench 及派生接触状态一起编码进冻结视频骨干，联合生成动作 + 描述未来事件与力学的"contact package"（**不是触觉像素**）；Haptic-Imagination Distillation 把两者迁到一个**去掉指尖输入分支**的学生上。**3 个真机接触密集抓放任务，学生（推理时无触觉输入）逐任务成功率 77%**。**对没有触觉硬件的团队，这是一条可评估的路径**（对照 #222 ViTaR / A6 TacPAC——那两条都需要真触觉在环） |真机 |
| A15 | 把世界模型表示蒸进紧凑策略：**部署成本与未蒸馏基线逐位相同** | [2609.24682](https://arxiv.org/abs/2609.24682) | 09-21（v2 09-22） | Arch, Edge | 论点：世界模型的 grounding 活在内部特征里，"生成未来"只是产生这些特征的目标函数——所以可以继承 grounding、丢掉每次决策要花几秒的生成机构。做法：在普通 VLA 训练上**加一个特征对齐项**，对着一个在训练帧上**跑一次并缓存**的冻结世界模型；训练时不加载教师，投影头训完丢弃。**卖点数字：部署后的策略在消费级 RTX 5090 上是 32ms / 1.86GB，与未蒸馏基线成本逐位相同**。未取到成功率 | 未确认 |
| A16 | No Free Checker：约 **150 个验证器**的综述，把「可得性」与「可信度」摆成互相拉扯的两轴 | [2609.09250](https://arxiv.org/abs/2609.09250) | 09 | Strategy | 覆盖成功检测器、奖励模型、运行时监控、安全过滤器、时序逻辑规范。两个互相冲突的属性：**可得性**（多便宜、rollout 里多早能给、判决多稠密）与**可信度**（高分究竟说明了多少——随着判决变得可被 game 且自利而下降）。标题即命题：**两者不可兼得**。31 页 / 187 篇引用。**给 #272 FailBench 提供框架，反之 FailBench 是可信度这一侧的经验实例**；也是 #209 奖励模型路线的必要对冲阅读 | — |
| A17 | AdaVLA：按 flow matching **轨迹曲率**估难度，在线自适应调步长与剪枝（训练无关） | [2608.29208](https://arxiv.org/abs/2608.29208) | 2026-08-29 | Edge | ⚠️ **本表唯一一条被明确降级的条目**。已核到的部分：论文的 Jetson AGX Orin 延迟表是 **π0.5（958.54 → 513.35ms）与 X-VLA（387.52 → 173.09ms）**，SmolVLA 只作为 **SO-ARM101 上的真机策略**出现。**流传的「SmolVLA on Orin 806.73 → 488.49ms」未能在正文中确认，且其归属与论文的具名表格不符——引用前必须自己打开 PDF 核对**。"IROS 2026 已接收"同样未能直接确认（abs 页 comments 字段取不到） | 仿真 + 真机（数字降级） |

**未展开但 ID 已确认、本期留作线索**（下期若有相关需求再深挖）：2609.06508 VLA-Corrector、2609.04277 FailureSpot、2609.30092 Self-Adaptive VLA、2609.16641 SAVLA、2609.24118 CARE、2609.22829 Whole-Body UMI、2609.19846 跨本体潜动作模型、2609.12075 面向机器人工厂的 VLA serving、2609.00908 自适应动作分块、2609.23650 任务语义动作校准、2609.21022 实时反馈去噪、2609.18108 GR00T N1.7 的一步漂移动作头。

**窗口边界说明**：FlashVLA（2608.27384，≥30Hz 单卡流式动作缓冲）提交于 2026-08-27，**在上期 v3.12 已收（#236）**，本期不重复。AdaVLA（2608.29208，08-29）是本期唯一保留的 2608 条目。

---

## 上期 arXiv 速查（2026-08-20 → 2026-08-27 窗口，仅核对到摘要）
...
## GitHub Issues 现场速查（2026-09 窗口，标题级）

> 本期主线是**"加载/配置失败被改道成一个看起来加载成功的模型"**——同一失败类被两位报告者用可复现步骤 + 数字记录了四次（#4577 / #4647 / #4711，加上 v3.11 的 #4415 与本期 PR #4601 作为旁系）。
> 已在上表写成完整条目的：#4614（→#253）、#4577（→#254）、#4647（→#255）、#4711（→#256）、PR #4452（→#257）、#4524（→#258）、#4679（→#259）、PR #4454（→#260）、#4727 / #4626 / #4663 / #4749（→#261）。下面是本期其余的标题级线索。

| Issue / PR | 标题 | 为什么值得跟 |
|---|---|---|
| [#3832](https://github.com/huggingface/lerobot/issues/3832) | LeRobot Community Roadmap 0.7.0 | **维护者自己列出的静默故障清单**，比任何第三方审计都直接：**DATA-17**「Missing images, buffer overflows, and streaming failures must fail or warn loudly instead of silently contaminating episode video」（WIP #4533）＝官方承认当前录制会静默污染；**DATA-04** 视频时间戳漂移（整数帧号 vs 严格 1/fps 倍数尚未定案）；**DATA-05** 零形状特征在 record/load/process/train 四处行为不一致；**DATA-06** 特征名/形状/dtype/角色没有单一真相源；**RUN-04**「Replace unsafe pickle-based transport with SafeTensors plus JSON」+ 消除标准推理与远程推理的行为差异；**TRAIN-01/02** 预训练策略契约（state/image 顺序、动作空间、归一化）**尚未规范**，且明确写了策略当前会"unintentionally overriding rename maps"；**ARCH-05** 相对动作无法独立于 policy 选择开启「with correct and understandable statistics」。另：**EVAL-01** 榜单要用 Wilson 置信区间 + Fisher 精确检验；**ARCH-11** 计划用 Trackio 替换 `wandb`；新策略/世界模型信号 **TRAIN-13 Cosmos 3**（WIP #3745）、**TRAIN-12 T-Rex**（NVIDIA 触觉策略）、**RL-05 SONIC**、**RL-11 π0.6 RECAP RL**、**RUN-06 PiR2** + 训练期 RTC（#4427）。唯一评论是 magus-robotics 提的触觉传感贡献（`TactileDataType`: displacement/force/full/wrench + paxini 串口后端 + 27 个无硬件测试） |
| [PR #4601](https://github.com/huggingface/lerobot/pull/4601) | fix(config): keep list-valued YAML overrides when loading a policy from a path | **用 YAML 配置的人必看的静默配置丢失**：`_flatten_to_cli_args()` 跳过了所有 list 值，于是写在 `policy.path` 旁边的序列型字段**从来没到达** `PreTrainedConfig.from_pretrained()`，checkpoint 里的值静默胜出。修法是把值 `json.dumps` 而不是丢掉 |
| [PR #4570](https://github.com/huggingface/lerobot/pull/4570) | fix(robots): release cameras and bus when connect() fails partway | 新增 `Robot._release_after_failed_connect()`；清理失败只记日志不抛，以免掩盖原始错误。**解释了经典的"连接失败后重试相机被占用"** |
| [PR #4528](https://github.com/huggingface/lerobot/pull/4528) | fix(streaming): use video-file-relative timestamps | #4524（→#258）的修复，确认了两种失败模式：**一个解码报错，一个图像被静默配上错误的 state/action** |
| [PR #4686](https://github.com/huggingface/lerobot/pull/4686) | fix(dagger): honor episode limit in autonomous recording | `num_episodes` 由定时保存触达时循环退出并**立刻对已重置的 buffer 再调一次 `save_episode()`**，writer 的空 buffer 校验异常被**静默吞掉**。修法是用 `has_pending_frames()` 门控最后一次保存 |
| [PR #4717](https://github.com/huggingface/lerobot/pull/4717) / [#4718](https://github.com/huggingface/lerobot/pull/4718) | Fix/rtc chunking seams 2 / 3 | 承接 PR #4454（→#260），RTC 拼接缝还在继续修——**真机跑 RTC 的人应当跟到收敛再升级** |
| [#4650](https://github.com/huggingface/lerobot/issues/4650) | [RDA Audit] Data Quality Report for `lerobot/svla_so101_pickplace` | 第三方审计工具（#234 RDA CLI）**开始对官方数据集出报告**：50 episodes / 11,939 frames。数据集审计从工具变成公共流程的第一步 |
| [#4665](https://github.com/huggingface/lerobot/issues/4665) | feat(datasets): add storage-format-aware writer routing and a native Lance write backend | 提案描述里明确说当前 main 有"a real silent-corruption path"；同时诚实标注 **Lance 并不替代采集端对丢帧、缺图、编码器失败、buffer 溢出的校验** |
| [#4390](https://github.com/huggingface/lerobot/issues/4390) | `mujoco>=3.4.0` silently breaks the initial-state premise of libero_spatial task 5 | 开在窗口之前（8 月），但**现在跑 LIBERO 的人必看**：碗在 reset 时不再稳坐在 ramekin 上，而 `<3.9.0` 的 pin **拦不住这个**。由 #4614 引用而确认 |
| [#4415](https://github.com/huggingface/lerobot/issues/4415) / [#4451](https://github.com/huggingface/lerobot/issues/4451) | Pipeline 迁移后多数据集模型归一化被静默跳过 / 迁移把含下划线的特征名弄坏（如 `observation.environment_state`） | v3.11 已收（#227 一线），**本期需要更新的状态是：两条都仍然 open**，而它们正是 #4577/#4647/#4711 这一簇的直系祖先 |

**其他仓的本期线索**（非 lerobot-core）：`Physical-Intelligence/openpi` 有一簇围绕 `compute_norm_stats` 的 issue（#662 lerobot 转换后图像变成含 `byte`/`path` 键的 dict 导致 norm-stats 计算失败、#481 LIBERO 微调分片维度错误、#675 从 S3 载 FAST base checkpoint 的 norm stats 时不清楚该用哪个 asset ID、#711、#773，以及 PR #999 单设备分片）——**未能确认这些是否落在本窗口内，按未定期线索收录，不作为本期情报**。
...
*v3.13 — 2026-09-26 更新。Blog 275 条（#1-#275）+ Discord 154 条（D1-D154）+ GitHub 速查 21+10 条 + 本期 arXiv 速查 17 条，共 487 条。窗口 2026-08-28 → 2026-09-25（四周补采——上次运行在 08-28，本次为该定时任务此后的首次成功运行）。*

***v3.13 三条主线**：*

*1. **「加载器把失败伪装成成功」从个案簇升级为一个有名字的失败类，而且有了通用的检测手段**。v3.11-v3.12 的 #227 / #229 / #231 是四次独立个案；本期两位报告者用可复现步骤把它记录了四次并各带数字：**#254** `PI0Policy.from_pretrained(strict=True)` 加载失败仍 `return model`，**37 个张量全零**而模型照样输出合理动作（报告者"一整批测量全部作废"）；**#255** `PreTrainedConfig.from_pretrained()` 不回填 `pretrained_path` → 归一化统计量为空 → `NormalizerProcessorStep` 退化成恒等映射、原始 0-255 像素直灌网络，PushT 上 **cum 4.8 vs 118-199**，唯一线索是**一条 warning**；**#256** `groot` 的裸 `except Exception` 让**瞬时网络错误与"文件不存在"无法区分**、`wall_x` 的 `strict=False` 不报 key。**本期最值得直接抄进所有人 eval 脚本的一行**（出自 #254 评论）：**加载后在固定 batch 上断言输出分布与全新随机初始化的模型不同**；配套 #255 的诊断手段：**dump normalizer 的 `state_dict()` 看是不是空的**。而 roadmap #3832 里维护者自己写下的 **DATA-17「must fail or warn loudly instead of silently contaminating episode video」**、**TRAIN-01/02「预训练策略契约尚未规范」**，说明这不是若干 bug 而是**契约缺失**——这一条会持续发作到 0.7.0。*

*2. **「加速不是免费的，但代价往往不在你以为的地方」**。三条互相独立的证据落在同一周：**#269** SmolVLA 转 ONNX，p99 从 1181ms 砍到 601ms 而 LIBERO Spatial 从 70.0% 掉到 41.0%——**但正文交叉核对推翻了最顺口的那个解释**：真凶是**导出器默认 16 token 的语言宽度截断了 10 条指令中的 5 条**，改成 24 就回到 75.0% 而延迟几乎不变（583.6 → 587.7ms），而且那个"INT8"产物**经 graph audit 是逐字节相同的 FP32 图**。**#270** VLAQuantBench 则给出反直觉的非单调性：**把 π0.5 action head 里被量化的层从 126 扩到 167，成功率从 7.0% 涨到 70.5%**（仍低于未量化基线 28.5pp）。**#267** 官方 Isaac-GR00T 文档给出诚实的代价标价：TRT 全管线 H100 上 **3.08× 加速**，但 **LIBERO 闭环 100% → 95%**。**读者行动很明确：导出/量化后第一件事不是看延迟，是跑闭环 + 检查 tokenizer 宽度有没有把指令切掉**（#269 那条修复几乎零成本）。这三条与 #3（量化 flow matching action expert 严重降精度）不矛盾，而是把它细化成了"层范围 × 格式 × 校准 × 接口"四个维度。*

*3. **预训练/数据这条线第一次有了两个方向相反的干净刻度**。正面：**#262 Figure Helix 2.5** 给出**少见的"一切其他变量固定"的预训练消融——随机初始化 9% vs Index 预训练 56%**，并在 **30 户未见过的真实住宅**里零样本跑 3 个全身行为（全有或全无评分、任何安全干预直接判失败）；它还把人→人形迁移的 scaling law 用到了**提前把最大那次 run 的留出 loss 预测到小数点后 4 位、误差是 8× 数据范围内变化量的 0.54%**。反面：**#273 GE-Act 2.0** 把共训数据从 **300 小时加到 30,000 小时（100×），G1-OP 真机 100 任务的零样本成功率只从 17.1% 到 44.1%**，而且仍有 24 个任务是零。**两条一起读的结论**：预训练是真的有决定性（6× 差距）、门槛也是真的（#213 #220 #223 的 3-4 万小时），**但门槛之上的边际收益很薄，所以数据质量手段开始比数据量手段更值钱**——本期 **#275 RoboDrop**（梯度相容性筛掉坏演示，4 任务真机 **35.0% → 63.75%**，HANG CUP 2/20 → 9/20）和 **TraceFlow**（**只用终局成功/失败一个 bit**、零权重更新，真机有序装箱 21/50 → 39/50 → 47/50，顺序错误从 20 降到 0）正是这个方向的第一批可自动化工具。*

***其他值得单独记住的**：**#257** 是本期最危险的一条——sync 与 RTC 推理引擎的关节顺序不一致，会让 **sync 静默地用另一个关节的值指挥每一个关节**，而在镜像对称双臂上叠加相对动作后**复合成 runaway 而不是固定偏移**（作者复盘原话：「a happy-path-only test is what let this land」）；**#253** 官方 `lerobot/smolvla_libero` 自带 `n_action_steps=50` 直接吃掉 **20-23pp**（McNemar p=0.031 / 2e-7），而同名的 `HuggingFaceVLA/` 仓发的是 1——**改一个 CLI 参数换回 20pp，本期性价比最高的修复**，同 issue 还揭出 `--env.fps` 会静默改仿真控制率（`fps=30` 实跑 31.25Hz）；**#259** `lerobot-record` 存的是请求动作而非限幅后实际下发的动作，**你在用机器人从未执行过的指令训练策略**；**#263** Skild 的部署铁律「**99.9% 准确但慢 10 倍不是差一点能部署，是不能部署**」+ 煮蛋"第一次成功 1 周、可靠 2 个月"；**#265 FLUX 3 Action** 是 WAM 路线第一次有主流开源权重（7B，RoboLab-120 **42.92%** 领先 Cosmos3-Nano-Policy 36.8% 与 DreamZero 25.7%），且贡献了一个值得抄的方法论——**用自研电子游戏当快速评测台，因为"在硬件上评一个 checkpoint 要一整个下午"**；**#272 FailBench** 给奖励模型路线（#209）泼了必要的冷水：**接触密集装配任务上没有任何 VLM 检测器超过 0.60 balanced accuracy，而且专门为失败检测微调过的模型持续不如通用 VLM**。*

***纠偏项**：**#268** GR00T N2 **至今未发布**（2026-03-16 GTC 的"预览"，原话"预计年底可用"），其"MolmoSpaces / RoboArena 第一"是 3 月的 `currently` 且已被本期 #265 的数据打上问号；">2×" 在新闻稿里**没有任何方法论**。另外本期修正了两处自己初稿的错误并保留了痕迹（#269 的"INT8 不是 INT8"、#270 的量化方向），**作为"摘要级采集必须交叉核对正文"的实例留档**。*

***采集缺口与说明**：(1) **Discord 连续第四期未采集**——本次是**定时任务的非交互运行，浏览器面板对 `huggingface.co` / Discord 的站点授权无人可批**，因此 HF Blog 列表页与 Discord 均未能用浏览器打开；**这条阻塞不会靠"下次记得登录"解决，需要在交互式会话里预先把站点授权设为 `site` 范围**。(2) **HF Blog 侧本期只捞到 #265 FLUX 3 Action 一条**（经 `web_fetch` 直取成功）；`?tag=robotics` 列表页因 provenance 限制取不到。**一个需要更新 skill 的发现：`mcp__workspace__web_fetch` 现在能正常抓 `huggingface.co`，CLAUDE.md §6 与本 skill 里"WebFetch 对 huggingface.co 被 EGRESS_BLOCKED"的说法已过时**；真正的限制换成了 **provenance set——URL 必须先出现在 WebSearch 结果或前一次 fetch 结果里**，所以每个新域名要预算一次额外的搜索往返。(3) **Physical Intelligence 博客连续第三期无新文**，最新仍为 2026-04-16 π0.7。(4) **Google DeepMind 机器人博客本期无新文**（最新 2026-07-30 Gemini Robotics 2，已直接核对博客首页）；AMD ROCm 机器人标签最新 2026-06-17；phospho 博客已休眠（最新 2025-06）；**Generalist AI 的 GEN-1.5 / "Embodied Foundation Models are One-Shot Learners" 发在 2026-08-19，差 9 天落在窗口外**（声称 12 秒演示即可 in-context 学会新任务），下期若有需要可回补。(5) **Agility / 1X / Unitree / AgiBot / Galbot 本期为"仅搜索未直取"的弱否定**——没有直接抓它们的博客首页，如果在意需要补一轮。(6) **本期 arXiv 速查表（A1-A17）全部数字取自摘要，未展开正文**；#269-#275 是唯一做过正文/全文核对的一批。**A17 AdaVLA 的延迟数字已明确降级**：流传的"SmolVLA on Orin 806.73 → 488.49ms"与论文具名表格（π0.5 958.54 → 513.35ms、X-VLA 387.52 → 173.09ms）归属不符，引用前必须自己开 PDF。(7) **本 skill 的协议文件路径已失效**：CLAUDE.md §6 与定时任务都指向 `scripts/en-vla-collector/english-vla-collector-SKILL.md`，**该文件不存在**；实际协议在 `.qoder/skills/en-vla-collector.md`，本期按后者执行。*
